...
res_path = os.path.join('independence_results', 'results.json')
...
for dataset in data_list:
    ...
    for col in job_cols:
        ...
        if len(df) != len(df_remain) + len(df_sampled):
            ...
            quit()
        for rep in reps:
            if job_exists(results, dataset['file_name'], col, rep):
                print(f"Already did {rep} replicates for {dataset['file_name']} and column {col}")
                continue
            print(f"Do {rep} replicates")
            df_repped = append_replicates(df_remain, df_sampled, rep)
            if len(df_repped) != len(df_remain) + (len(df_sampled) * rep):
                print(f"Error: Length mismatch for {dataset['file_name']} and column {col} with {rep} replicates")
                quit()
            prec = calculate_precision(df_repped, df_sampled, col)
            results.append({
